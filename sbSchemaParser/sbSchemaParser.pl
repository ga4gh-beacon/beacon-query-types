...
=cut
  ...
  my %cats			=		map{ $_ => 1 }
										grep{ /\w/ } ( 
											@{ $config->{categories} },
											@{ $config->{schema_repos}->{$paths->{schema_repo}}->{categories} }
										);
  my %tags			=		map{ $_ => 1 }
  									grep{ /\w/ } ( 
  										@{ $config->{tags} },
  										@{ $config->{schema_repos}->{$paths->{schema_repo}}->{tags} }
  									);
...
foreach (sort keys %cats) { $header .= "  - ".$_."\n" }

$header 				.=	<<END;
tags:
END

foreach (sort keys %tags) { $header .= "  - ".$_."\n" }
